...
var SqlServerBase = function() {
    this.defaults = _.merge(this.defaults, {
        expand: false,
        limit: 100
    });
}
...
SqlServerBase.prototype.addWhereClause = function(req, queryInfo, queryString) {
    ...
    for (var key in query) {
        if (query.hasOwnProperty(key) && key !== 'expand' && key !== 'limit' && key !== 'orderby' && key !== 'apikey') {
    ...
    if (queryInfo.queryParameters) {
        for (var key in nonstockQueryParameters) {
            ...
            if (whereFragment) {
                ...
                queryString = this.addWhere(queryString, templatedWhereFragment);
            } else if (queryInfo.inferWhereParams) {
                var templatedWhereFragment;
                if (key.length > 5 && key.substring(key.length - 5, key.length) === '__not') {
                    var col = key.slice(0, -5);
                    templatedWhereFragment = col + ' != \'' + nonstockQueryParameters[key] + '\'';
                    queryString = this.addWhere(queryString, templatedWhereFragment);
                } else if (key.length > 10 && key.substring(key.length - 10, key.length) === '__not_like') {
                    var col = key.slice(0, -10);
                    templatedWhereFragment = col + ' not like \'' + nonstockQueryParameters[key] + '\'';
                    queryString = this.addWhere(queryString, templatedWhereFragment);
                } else if (key.length > 6 && key.substring(key.length - 6, key.length) === '__like') {
                    var col = key.slice(0, -6);
                    templatedWhereFragment = col + ' like \'' + nonstockQueryParameters[key] + '\'';
                    queryString = this.addWhere(queryString, templatedWhereFragment);
                } else if (key.length > 4 && key.substring(key.length - 4, key.length) === '__gt') {
                    var col = key.slice(0, -4);
                    templatedWhereFragment = col + ' > \'' + nonstockQueryParameters[key] + '\'';
                    queryString = this.addWhere(queryString, templatedWhereFragment);
                } else if (key.length > 4 && key.substring(key.length - 4, key.length) === '__lt') {
                    var col = key.slice(0, -4);
                    templatedWhereFragment = col + ' < \'' + nonstockQueryParameters[key] + '\'';
                    queryString = this.addWhere(queryString, templatedWhereFragment);
                } else{
                    templatedWhereFragment = key + ' = \'' + nonstockQueryParameters[key] + '\'';
                    queryString = this.addWhere(queryString, templatedWhereFragment);                }
            }
        }
    }
